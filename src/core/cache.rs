use super::files::{MARKDOWN_EXTENSIONS, ASSET_EXTENSIONS, collect_markdown_files, collect_asset_files, ASSET_BINARY_EXTENSIONS};
...
fn compute_target_for_source(src: &Path, cwd: &Path, is_binary: bool) -> (PathBuf, PathBuf) {
    ...
    let target = if is_binary {
        ...
        let ext = if fmt.eq_ignore_ascii_case("bincode") { ".bin" } else { ".json" };
...
impl MarkdownCache {
    ...
    pub fn new<P: AsRef<Path>>(entry_dir: P) -> io::Result<Self> {
        ...
        // optionally assets under "public" (binary/text depending on extension).
        ...
        // Default assets dir is "public" at project root if present.
        let cwd = env::current_dir()?;
        let assets_dir: Option<PathBuf> = {
        ...
        for (src, content) in &cache_map {
            let (target, _rel) = compute_target_for_source(src, &cwd, matches!(content, CachedContent::Binary(_)));
                                ...
                                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                                    ...
                                    if MARKDOWN_EXTENSIONS.contains(&ext_l.as_str()) {
                                        match fs::read_to_string(&path) {
                                            Ok(text) => {
                                                if let Ok(mut w) = cache.write() {
                                                    w.insert(path.clone(), CachedContent::Text(text.clone()));
                                    ...
                                    // Asset file
                                        ...
                                        let is_binary = ASSET_BINARY_EXTENSIONS.contains(&ext_l.as_str());
                                        if is_binary {
                                            match fs::read(&path) {
                                                Ok(bytes) => {
                                                    if let Ok(mut w) = cache.write() {
                                                        w.insert(path.clone(), CachedContent::Binary(bytes.clone()));
                                                        if let Err(e) = tx.send(PersistCommand::Update(
                                                            path.clone(),
                                                            CachedContent::Binary(bytes),
                                                        )) {
    ...
    /// Returns Some(text) for textual cache entries; binary entries return None.
    pub fn get(&self, path: &Path) -> Option<String> {
        match self.cache.read() {
            Ok(r) => match r.get(path) {
                Some(CachedContent::Text(s)) => Some(s.clone()),
                Some(CachedContent::Binary(_)) => None,
                None => None,
            },
            Err(_) => None,
        }
